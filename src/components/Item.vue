        ...
        <ul class="store_list" v-if="totalNum !== 0">
            ...
            >
                <div
                    :class="[
                        'store_wrapper',
                        store.store_promotion.length === 0
                            ? ''
                            : 'border_1px_bottom',
                    ]"
                >
                    <span class="store_basic">
                        ...
                        <i
                            ...
                            :style="{
                                backgroundImage: `url(${getStoreType(
                                    store.store_type,
                                )})`,
                            }"
                        ></i>
                    ...
                    <span class="store_attach">
                        ...
                        <span class="get_edit" @click="toggleDelete(storeIndex)"
                            >编辑</span
                        >
                    </span>
                </div>
                ...
                <ul class="commodity_list">
                    <li
                        ...
                        v-for="(commodity,
                        commodityIndex) in store.commodity_list"
                        :key="commodity.sku_id"
                    >
                        <div
                            class="commodity_item_container"
                            :id="
                                `commodityItem_${store.store_id}_${commodity.sku_id}`
                            "
                            v-touch:swipe="
                                handleSwiper(store.store_id, commodity.sku_id)
                            "
                        >
                            <div class="radio_img_wrapper">
                                <label
                                    :for="
                                        `selectStoreOne_${store.store_id}_${commodity.sku_id}`
                                    "
                                    class="radio_label"
                                    @change="
                                        toggleCommodity({
                            ...
                            <div class="commodity_detail_wrapper">
                                ...
                                <div
                                    v-for="(warning,
                                    index) in commodity.warning_msg"
                                    :key="index"
                                >
                                    <p class="warning low_stock_warning">
                                        {{ warning }}
                                    </p>
                                </div>
            ...
            <li class="store_item" v-if="invaildCommodities.length > 0">
                ...
                <ul class="commodity_list">
                    ...
                    >
                        <div
                            class="commodity_item_container invaild_item_container"
                        >
                            <span
                                :class="[
                                    'invaid_logo',
                                    commodity.pre_hot ? 'pre_hot' : '',
                                ]"
                            ...
                            <div class="invaid_detail_wrapper">
                                ...
                                <p class="invaid_reason">
                                    {{ commodity.fail_reason }}
                                </p>
...
export default class Item extends Vue {
    ...
    get totalPrice() {
      return this.selectList.reduce(
        (val, acc) => acc.curNum * acc.unitPrice,
        0,
      );
    }
    ...
    public toggleStore(storeId: string) {
      ...
      if (currStore) {
        if (isStoreSelected) {
          const selectedSkuIdsByStore = this.getSelectedSkuIdsByStore(
            storeId,
          );
          this.selectList = this.selectList.filter(
    ...
    public handleSwiper(storeId: string, skuId: string) {
      return (direction: string) => {
        ...
        const curDeleteDOM = document.querySelector(
          `#delete_${storeId}_${skuId}`,
        );
        if (direction === 'left') {
    ...
    public getDialogFeedback(type: number, confirmToDelete: boolean) {
      if (type === 1) {
        if (confirmToDelete) {
          this.invaildCommodities.splice(
            0,
            this.invaildCommodities.length,
          );
        ...
        if (
          this.vaildCommodities[this.curStoreIndex].commodity_list
            .length === 0
        ) {
