  ...
  methods: {
    ...
    addCount(storeIndex, commodityIndex, curNum, limitMaxNum, maxStockNum, skuId) {
      ...
      if (limitMaxNum > 0) {
        ...
        if (curNum + 1 > limitMaxNum) {
          ...
          this.vaildCommodities[storeIndex].commodity_list[commodityIndex].cur_cart_num += 1;
    ...
    getDialogFeedback(type, params) {
      if (type === 1) {
        ...
        this.vaildCommodities[this.curStoreIndex].commodity_list.splice(this.curSkuIndex, 1);
        if (this.vaildCommodities[this.curStoreIndex].commodity_list.length === 0) {
